...
for h = hiddenSizes
    % Define search space for AE regularization params
   % Hyperparameter ranges
l2Regs = [0.0001, 0.001, 0.01];
sparsityRegs = [0.1, 1, 4];
sparsityProps = [0.05, 0.1, 0.2];
hiddenSize = 20;  % or any chosen size

bestReconError = inf;

% Normalize data once (before loop!)
[X_train_norm, mu, sigma] = zscore(X_train);

for l2 = l2Regs
    for spReg = sparsityRegs
        for spProp = sparsityProps
            % Train AE with given hyperparameters
            ...
            X_train_ae_temp = encode(autoenc, X_train_norm')';
            X_test_norm = (X_test - mu) ./ sigma;
            X_test_ae_temp = encode(autoenc, X_test_norm')';

            % Reconstruction of training data
            X_recon = predict(autoenc, X_train_norm');
            
            % Corrected computation of reconstruction error
            reconError = mse(X_train_norm' - X_recon);

            % Update best parameters based on reconstruction error
            if reconError < bestReconError
                bestReconError = reconError;
                X_train_ae = X_train_ae_temp;
                X_test_ae = X_test_ae_temp;
                bestAEParams = struct('L2', l2, 'SparsityReg', spReg, 'SparsityProp', spProp);
                
                fprintf('Updated Best AE Params: L2=%.4f, SparsityReg=%.2f, SparsityProp=%.2f, ReconError=%.5f\n', ...
                    l2, spReg, spProp, reconError);
            end
        end
    end
end
...
function [X_train_ae, X_test_ae, autoenc] = autoencoder_features_custom(X_train, X_test, hiddenSize, l2, sparsityReg, sparsityProp)
    [X_train_norm, mu, sigma] = zscore(X_train);
    X_test_norm = (X_test - mu) ./ sigma;
