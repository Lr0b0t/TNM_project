...
switch lower(dim_reduc)
    case 'vectorized'
         [X_train, feature_labels, idx_mask] = vectorize_rdcm_train(fc_train); % Use TRAIN ONLY
          X_test  = vectorize_rdcm_test(fc_test, idx_mask);  % Use the same mask for test set
    case 'pca'
        [X_train, feature_labels, idx_mask] = vectorize_rdcm_train(fc_train); % Use TRAIN ONLY
         X_test  = vectorize_rdcm_test(fc_test, idx_mask);  % Use the same mask for test set
